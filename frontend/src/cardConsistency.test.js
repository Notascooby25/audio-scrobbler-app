...
describe('Card Border Consistency & Flat-By-Default Audit', () => {
  ...
  it('contains zero non-none box-shadows across styles.css (excluding focus rings and hairline badge outlines)', () => {
    ...
    for (const declaration of boxShadowMatches) {
      expect(declaration).toMatch(/box-shadow\s*:\s*(none|0\s+0\s+0\s+0\.5px\s+#E5E5E5)\s*;/)
    }
  })
